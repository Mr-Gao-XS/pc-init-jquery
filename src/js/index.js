...
var helpList = []
// 遍历帮助列表里的高度
$('.helpbox').each((index, el) => {
    var top = $(`.helpbox:eq(${index})`).offset().top - $(`.helpbox:eq(${index})`).outerHeight() - 400
    helpList.push({ 'offTop': top, 'index': index })
})
...
function scrollrecommended() {
    ...
    if ($('.recommendedContent').offset()) {
        ...
        if (topScroll >= recommend) {
            $('.recommendedContent li').each((index, el) => {


                $(`.recommendedContent li:eq(${index})`).addClass(`move-top${index + 1}`)
            })
        }
    }
}
function scrollproblem() {
    if ($('.problem').offset()) {
        ...
        helpList.forEach((res, index) => {
            if (topScroll >= res.offTop) {
                $(`.helpbox:eq(${res.index})`).find('.helpleft').addClass('move-left')
                $(`.helpbox:eq(${res.index})`).find('.helpright').addClass('move-right')
            }
        })
    }
}
...
$(document).on('click', '.analysis-swiper-pagination  .swiper-pagination-bullet', function () {
    ...
    if (index >= 3) {
        $('.analysis-swiper-pagination').animate({ scrollLeft: (158 * (index - 3) + 79) }, 300);
    } else {
...
if ($('.movebox')) {
    $('.movebox').find('img').each(function (index, el) {
        ...
        _this.on({
            mouseover: function () {
                allclear()
                $(`.Picindex:eq(${index})`).stop().fadeIn('normal')
...
function allclear() {
    $('.movebox').find('img').each(function (index, el) {
        var _this = $(this)
        $(`.Picindex:eq(${index})`).stop().fadeOut('normal')
...
function initTreeScroll() {
    ...
    $(window).scroll(function () {
        ...
        // console.log(treeTop, winTop, winHeight)
...
$('.login-phone,.login-code').on({
    ...
    focus: function () { $(this).parent('.Login-item').css('border', '1px solid #3c96ff') },

    blur: function () { console.log(123); $(this).parent('.Login-item').css('border', '1px solid #dfe2e6') }
